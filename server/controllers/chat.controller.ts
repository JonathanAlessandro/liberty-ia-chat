...
export async function askChatQuestion(input: {
  ...
  const conversation = await findOrCreateConversation(input.visitorId, input.conversationId);
  const history = await listConversationMessages(conversation.id, input.visitorId);
...
export async function getChatHistory(conversationId: number, visitorId: string) {
  return listConversationMessages(conversationId, visitorId);
}
